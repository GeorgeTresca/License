...
def load_q_table():
    ...
    if os.path.exists(q_table_file):
        ...
        raise ValueError("No trained model found for pescatarian.")


def filter_pescatarian_meals():
    banned_ingredients = {
        ...
        "turkey slices", "bacon", "ground beef", "ground turkey", "pulled pork", "bacon", "steak"
    }
...
def recommend_meals_for_all_calories():
    ...
    pescatarian_meals = filter_pescatarian_meals()

    results = {}

    for target_calories in CALORIE_RANGES:
        if target_calories not in q_table:
            print(f"No trained Q-values for {target_calories} kcal in pescatarian model")
            continue

        state = []
        current_calories = 0
        available_meals = list(range(len(pescatarian_meals)))
        num_meals = 4
        recommendations = []
